...
class Fleet {
  ...
  /** Prints the graphviz visualization to filename.gv */
  def printNetwork(filename: String, quiet: Boolean = false) {
    printNetwork(new PrintWriter(new File(filename + ".gv")), quiet)    
  } 
  /** Writes visualization to f. (Testing only!) */
  def printNetwork(f: PrintWriter, quiet: Boolean) {
    f.println("""// Auto-generated by the graphviz module!
// Do not make manual edits. They will be overwritten next time run.
// Execute: neato -Tgif FILE.gv -o FILE.gif
digraph G {""")
    locs.foreach(l => f.println(l.gvString))
    segs.foreach(s => f.println(s.gvString))
    f.println("}")
    f.close()
    if (!quiet) println("Run: neato -Tgif FILENAME.gv -o FILENAME.gif")
  }
}
